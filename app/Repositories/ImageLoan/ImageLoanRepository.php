...
class ImageLoanRepository implements ImageLoanRepositoryContract
{
    public function upload( $form_data )
    {
        // p('hehe');exit;
        $validator = Validator::make($form_data, LoanImage::$rules, LoanImage::$messages);
        ...
        $sessionImage = new LoanImage;
        $sessionImage->filename      = $img_dir;
        // p($img_dir);exit;
        $sessionImage->original_name = $originalName;
        $sessionImage->loan_id        = $form_data['loan_id'];
        // dd('hehe');
        $sessionImage->save();
    ...
    {
        $full_size_dir = Config::get('imagesLoan.full_size');
    ...
    public function original( $photo, $filename )
    {
        ...
        $image = $manager->make( $photo )->resize((600*$img_bili), 600)->resizeCanvas(800, 600, 'center', false)->insert('images/water_img.png', 'bottom-right', 0, 2)->save(Config::get('imagesLoan.full_size') . $filename );
    ...
    public function icon( $photo, $filename )
    {
        ...
        $image = $manager->make( $photo )->resize(200, null, function ($constraint) {
            ...
            ->save( Config::get('imagesLoan.icon_size')  . $filename );
    ...
    public function delete($requestData)
    {
        ...
        $full_size_dir = Config::get('imagesLoan.full_size');
        $icon_size_dir = Config::get('imagesLoan.icon_size');
        $base_dir      = Config::get('imagesLoan.base_size');

        /*p($full_size_dir);
        p($icon_size_dir);*/
        // dd($requestData->all());
        if(empty($requestData->img_loan_id)){

            $sessionImage = LoanImage::where('original_name', $requestData->img_name)
                             ->orderBy('created_at', 'DESC')
                             ->first();
        }else{

            $sessionImage = LoanImage::where('filename', $requestData->img_name)
                             ->where('loan_id', $requestData->img_loan_id)
                             ->orderBy('created_at', 'DESC')
                             ->first();
        }
